...
export class GradeSubmissionService {
   ...
   */
  async approveSubmission(id: string, userId: string) {
    const submission = await this.gradeSubmissionModelAction.get({
    ...
    await this.gradeSubmissionModelAction.update({
      identifierOptions: { id },
      updatePayload: {
   ...
   * Reject a submission
   */
  async rejectSubmission(id: string, userId: string, reason: string) {
    const submission = await this.gradeSubmissionModelAction.get({
      identifierOptions: { id },
    });
    ...
    this.logger.info(sysMsg.GRADE_REJECTED, {
      submissionId: id,
    });

    return { message: sysMsg.GRADE_REJECTED };
  }
